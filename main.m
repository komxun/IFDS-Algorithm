...
% _______________________ SE(3) Controller Setup _________________________
% Ported from se3quad/matlab (Lee et al. 2010/2011, arXiv:1003.2005).
% Replaces CCA3D_2 path follower + kinematic UAV model with full rigid-
...
% UAV state: full SE(3)  (p, v, R, Omega)
state.p     = [x_i; y_i; z_i];
state.v     = zeros(3,1);
state.R     = eye(3);                     % start level, body-1 along +x
state.Omega = zeros(3,1);

% Dirty-derivative filters (acceleration / jerk of actual velocity).
% Handles are preserved across segments so memory persists.
filters.dv1dt = DirtyDerivative(1, P.tau,    P.Ts);
filters.dv2dt = DirtyDerivative(2, P.tau*10, P.Ts);

% Speed history (magnitude of linear velocity)
vhist = [norm(state.v)];

% SE(3) telemetry logger (appended to at every controller step).
% All arrays grow along columns; final size ~ tsim/P.Ts.
logger.t      = [];   % 1xN  global time                       [s]
logger.x      = [];   % 3xN  actual position                   [m]
logger.xd     = [];   % 3xN  desired position                  [m]
logger.v      = [];   % 3xN  actual velocity                   [m/s]
logger.vd     = [];   % 3xN  desired velocity                  [m/s]
logger.Omega  = [];   % 3xN  actual body-rate                  [rad/s]
logger.Omegac = [];   % 3xN  commanded body-rate               [rad/s]
logger.Psi    = [];   % 1xN  SO(3) geodesic error function
logger.f      = [];   % 1xN  total collective thrust           [N]
logger.M      = [];   % 3xN  moment vector                     [N.m]
logger.deltaF = [];   % 4xN  per-rotor forces  (Mix * [f; M])  [N]
...
%         timer(L) = toc;
    ...
    if foundPath ~= 1 || isempty(Paths{rt}) || size(Paths{rt},2)==1
        ...
        disp("*UAV is standing by — holding position (SE(3) hover)*")

        % SE(3) hover: xd = current position, zero ref velocity.
    ...
    % Accumulate dense SE(3) trajectory (all controller steps) so the
    % 3D plot is smooth rather than waypoint-spaced.
    trajectory = [x_i; y_i; z_i];

    dtcum = 0;
    
    for j = 1:size(Paths{rt},2)-1
        if dtcum >= dt_traj
            break
        end 
        Wi = Paths{rt}(:,j);
        ...
        if a*(x_i - Wf(1)) + b*(y_i - Wf(2)) + c*(z_i - Wf(3)) < 0
            ...
            % --- SE(3) geometric tracker (replaces CCA3D_2) ---------
            dt_budget = dt_traj - dtcum;            % remaining time budget
            [pos_seg, vm_seg, state, filters, timeSpent, logger] = ...
                SE3Track(Wi, Wf, state, filters, C, dt_budget, P, logger);
            x_i = state.p(1);  y_i = state.p(2);  z_i = state.p(3);
            dtcum = dtcum + timeSpent;

            % Append the dense segment trajectory (skip duplicate first
            % sample which equals the previous segment's last sample).
...
%             disp("skip waypoint #" + num2str(j)) 
    ...
    traj{rt} = trajectory;
    timer(rt) = toc;
    vhist(end+1) = norm(state.v);           % end-of-iteration speed magnitude
...
%% SE(3) Controller Telemetry (translational / rotational / actuators)
se3_plot(logger, P);
